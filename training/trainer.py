...
class Trainer:
    ...
    def _enable_gradient_checkpointing(self) -> None:
        ...
        if hasattr(self.model, "blocks"):
            for block in self.model.blocks:  # type: ignore[union-attr]
            ...
            try:
                ...
                self._wandb_run = wandb.init(
                    ...
                    config={
                        "model": {
                            "vocab_size": getattr(
                                self.model.config, "vocab_size", None
                            ),
        ...
        for epoch in range(self.config.epochs):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                if (
                    ...
                    and self.global_step % getattr(self.config, "val_every", 100) == 0
                ):
    ...
    def _log_training_step(self, loss: torch.Tensor, batch_idx: int) -> None:
        ...
        if self._tb_writer is not None:
            self._tb_writer.add_scalar(
                "train/loss", float(loss.item()), self.global_step
            )
            self._tb_writer.add_scalar("train/lr", lr, self.global_step)
            self._tb_writer.add_scalar(
                "train/perplexity", self.train_metrics.perplexity, self.global_step
            )
    ...
    def validate(self) -> float:
        ...
        if self._tb_writer is not None:
            ...
            self._tb_writer.add_scalar(
                "val/perplexity", self.val_metrics.perplexity, self.global_step
            )
